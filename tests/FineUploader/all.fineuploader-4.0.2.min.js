...
}, qq.version = "4.0.2", qq.supportedFeatures = function ()
{
	function a()
	{
		var a, b = !0;
...
{
	return qq.DeleteFileAjaxRequestor && this._options.deleteFile.enabled ? this._options.cors.expected ? qq.supportedFeatures.deleteFileCorsXhr ? !0 : qq.supportedFeatures.deleteFileCorsXdr && this._options.cors.allowXdr ? !0 : !1 : !0 : !1
...
{
	if (!c.success)if (d && 201 !== d.status && !c.error)this._options.callbacks.onError(a, b, "XHR returned response code " + d.status, d); else
	{
...
}, qq.FineUploaderBasic.prototype = qq.basePublicApi, qq.extend(qq.FineUploaderBasic.prototype, qq.basePrivateApi), qq.AjaxRequestor = function (a)
{
	"use strict";
	function b()
	{
	...
	var s, t, u = [], v = [], w = {validMethods: ["POST"], method: "POST", contentType: "application/x-www-form-urlencoded", maxConnections: 3, customHeaders: {}, endpointStore: {}, paramsStore: {}, mandatedParams: {}, successfulResponseCodes: {DELETE: [200, 201,202, 204], POST: [200, 201, 204]}, cors: {expected: !1, sendCredentials: !1}, log: function ()
	{
	}, onSend: function ()
	{
	}, onComplete: function ()
...
{
	this._deleteRetryOrCancelClickHandler = qq.DeleteRetryOrCancelClickHandler && this._bindDeleteRetryOrCancelClickEvent(), this._focusinEventSupported = !qq.firefox(), this._isEditFilenameEnabled() && (this._filenameClickHandler = this._bindFilenameClickEvent(), this._filenameInputFocusInHandler = this._bindFilenameInputFocusInEvent(), this._filenameInputFocusHandler = this._bindFilenameInputFocusEvent())
...
{
	var a = this;
	return new qq.DeleteRetryOrCancelClickHandler({templating: this._templating, log: function (b, c)
	{
		a.log(b, c)
...
{
	this._isEditFilenameEnabled() && this._templating.getFileContainer(a) && c !== qq.status.SUBMITTED && (this._templating.markFilenameEditable(a), this._templating.hideEditIcon(a))
...
{
	this._parent.prototype._onProgress.apply(this, arguments), this._templating.updateProgress(a, c, d), c === d ? (this._templating.hideCancel(a), this._templating.setStatusText(a, this._options.text.waitingForResponse), this._displayFileSize(a)) : this._displayFileSize(a, c, d)
...
{
	this._parent.prototype._onDeleteComplete.apply(this, arguments), this._templating.hideSpinner(a), c ? (this._templating.setStatusText(a, this._options.deleteFile.deletingFailedText), this._templating.showDelete(a)) : this._removeFileItem(a)
}, _sendDeleteRequest: function (a)
{
	this._templating.hideDelete(a), this._templating.showSpinner(a), this._templating.setStatusText(a, this._options.deleteFile.deletingStatusText), this._deleteHandler.sendDelete.apply(this, arguments)
...
}, qq.extend(qq.FineUploader.prototype, qq.basePublicApi), qq.extend(qq.FineUploader.prototype, qq.basePrivateApi), qq.extend(qq.FineUploader.prototype, qq.uiPublicApi), qq.extend(qq.FineUploader.prototype, qq.uiPrivateApi), qq.Templating = function (a)
{
	...
	function b()
	{
		...
		if (x("Parsing template"), null == Q.templateIdOrEl)throw new Error("You MUST specify either a template element or ID!");
		if (qq.isString(Q.templateIdOrEl))
		{
			if (a = document.getElementById(Q.templateIdOrEl), null === a)throw new Error(qq.format("Cannot find template script at ID '{}'!", Q.templateIdOrEl));
			b = a.innerHTML
		} else
		{
			...
			b = Q.templateIdOrEl.innerHTML
		}
		if (b = qq.trimStr(b), d = document.createElement("div"), d.appendChild(qq.toElement(b)), Q.button && (f = qq(d).getByClass(R.button)[0], f && qq(f).remove()), qq.DragAndDrop && qq.supportedFeatures.fileDrop || (i = qq(d).getByClass(R.dropProcessing)[0], i && qq(i).remove()), g = qq(d).getByClass(R.drop)[0], g && !qq.DragAndDrop && (qq.log("DnD module unavailable.", "info"), qq(g).remove()), g && !qq.supportedFeatures.fileDrop && qq(g).hasAttribute(N) && qq(g).css({display: "none"}), h = qq(d).getByClass(R.thumbnail)[0], E ? h && (P = parseInt(h.getAttribute(K)), P = P > 0 ? P : null, F = qq(h).hasAttribute(M)) : h && qq(h).remove(), E = E && h, z = qq(d).getByClass(R.editFilenameInput).length > 0, A = qq(d).getByClass(R.retry).length > 0, c = qq(d).getByClass(R.list)[0], null == c)throw new Error("Could not find the file list container in the template!");
		return e = c.innerHTML, c.innerHTML = "", x("Template parsing complete"), {template: qq.trimStr(d.innerHTML), fileTemplate: qq.trimStr(e)}
	}
	...
	function f(a)
	{
		return d(c(a), R.cancel)
	}

	function g(a)
	{
		return d(c(a), R.progressBarContainer) || d(c(a), R.progressBar)
	}

	function h(a)
	{
		return d(c(a), R.spinner)
	}

	function i(a)
	{
		return d(c(a), R.editNameIcon)
	}
	...
	function k(a)
	{
		return d(c(a), R.deleteButton)
	}

	function l(a)
	{
		return d(c(a), R.retry)
	}
	...
	function n()
	{
		return d(C, R.dropProcessing)
	}

	function o(a)
	{
		return E && d(c(a), R.thumbnail)
	}

	function p(a)
	{
		a && qq(a).addClass(Q.classes.hide)
	}

	function q(a)
	{
		a && qq(a).removeClass(Q.classes.hide)
	}

	function r(a, b)
	{
		var c = g(a);
		c && !qq(c).hasClass(R.progressBar) && (c = qq(c).getByClass(R.progressBar)[0]), c && qq(c).css({width: b + "%"})
	}

	function s()
	{
		var a = Q.placeholders.thumbnailNotAvailable, b = Q.placeholders.waitingForThumbnail, c = {maxSize: P, scale: F};
		E && (a && Q.imageGenerator.generate(a, new Image, c).then(function (a)
		{
			G = a
		}, function ()
		{
			x("Problem loading 'not available' placeholder image at " + a, "error")
		}), b && Q.imageGenerator.generate(b, new Image, c).then(function (a)
		{
			H = a
		}, function ()
		{
			x("Problem loading 'waiting for thumbnail' placeholder image at " + b, "error")
		}))
	}

	function t(a)
	{
		H ? (v(H, a), a.src = H.src, q(a)) : p(a)
	}

	function u(a)
	{
		G && !w(a) && (v(G, a), a.src = G.src, q(a))
	}
	...
	return qq.extend(Q, a), x = Q.log, C = Q.containerEl, E = void 0 !== Q.imageGenerator, B = b(), s(), y = {render: function ()
	{
		x("Rendering template in DOM."), C.innerHTML = B.template, p(n()), D = Q.fileContainerEl || d(C, R.list), x("Template rendering complete")
	...
	{
		var f = qq.toElement(B.fileTemplate), h = d(f, R.file);
		qq(f).addClass(J + a), h && qq(h).setText(b), f.setAttribute(I, a), c ? e(f, c.index) : D.appendChild(f), p(g(a)), p(j(a)), p(k(a)), p(l(a)), O && y.hideCancel(a)
	...
	{
		var b = m(a);
		b && qq(b).addClass(Q.classes.editable)
	...
	{
		return qq(a).hasClass(R.file)
	}, getButton: function ()
	{
		return Q.button || d(C, R.button)
	...
	{
		var b = i(a);
		b && qq(b).removeClass(Q.classes.editable)
	}, isEditIcon: function (a)
	{
		return qq(a).hasClass(R.editNameIcon)
	}, getEditInput: function (a)
	{
		return d(c(a), R.editFilenameInput)
	}, isEditInput: function (a)
	{
		return qq(a).hasClass(R.editFilenameInput)
	}, updateProgress: function (a, b, c)
	{
		var d, e = g(a);
		e && (d = Math.round(100 * (b / c)), b === c ? p(e) : q(e), r(a, d))
	...
	}, showCancel: function (a)
	{
		...
		{
			var b = f(a);
			b && qq(b).removeClass(Q.classes.hide)
		}
	...
	{
		return qq(a).hasClass(R.cancel)
	}, showDelete: function (a)
	{
		q(k(a))
	}, hideDelete: function (a)
	{
		p(k(a))
	}, isDelete: function (a)
	{
		return qq(a).hasClass(R.deleteButton)
	}, isRetry: function (a)
	{
		return qq(a).hasClass(R.retry)
	}, updateSize: function (a, b)
	{
		var c = j(a);
		c && (q(c), qq(c).setText(b))
	}, setStatusText: function (a, b)
	{
		var e = d(c(a), R.statusText);
	...
	{
		var c = o(a), d = {maxSize: P, scale: !0, orient: !0};
		if (qq.supportedFeatures.imagePreviews)
		{
			if (c)return t(c), Q.imageGenerator.generate(b, c, d).then(function ()
			{
				c.setAttribute(L, "true"), q(c)
			}, function ()
			{
				Q.placeholders.waitUntilUpdate || u(c)
			})
		} else c && t(c)
	}, updateThumbnail: function (a, b)
	{
		var c = o(a), d = {maxSize: P, scale: F};
		if (c)
		{
			if (b)return Q.imageGenerator.generate(b, c, d).then(function ()
			{
				q(c)
			}, function ()
			{
...
}, qq.UploadHandlerForm = function (a, b, c, d)
{
	...
	function f(b, c)
	{
		var d = a.paramsStore.getParams(b), e = a.demoMode ? "GET" : "POST", f = a.endpointStore.getEndpoint(b), i = h[b].newName || g.getName(b);
		return d[a.uuidParam] = h[b].uuid, d[a.filenameParam] = i, k.initFormForUpload({method: e, endpoint: f, params: d, paramsInBody: a.paramsInBody, targetName: c.name})
	}
...
}, qq.UploadHandlerXhr = function (a, b, c, d)
{
	...
	function g(b, c, d, e)
	{
		var f = new FormData, g = a.demoMode ? "GET" : "POST", h = a.endpointStore.getEndpoint(e), i = h, j = I[e].newName || F.getName(e), k = F.getSize(e);
		return b[a.uuidParam] = I[e].uuid, b[a.filenameParam] = j, N && (b[a.totalFileSizeParam] = k), a.paramsInBody || (N || (b[a.inputName] = j), i = qq.obj2url(b, h)), c.open(g, i, !0), a.cors.expected && a.cors.sendCredentials && (c.withCredentials = !0), N ? (a.paramsInBody && qq.obj2FormData(b, f), f.append(a.inputName, d), f) : d
	}
	...
	function n(a, b)
	{
		return 201 !== a.status || !b.success || b.reset
	}
...
}, qq.DragAndDrop = function (a)
{
	...
	function d(a, b)
	{
		...
		return h.callbacks.processingDroppedFiles(), b.dropDisabled(!0), a.files.length > 1 && !h.allowMultipleItems ? (h.callbacks.processingDroppedFilesComplete([]), h.callbacks.dropError("tooManyFilesError", ""), b.dropDisabled(!1), e.failure()) : (k = [], qq.isFolderDropSupported(a) ? qq.each(a.items, function (a, b)
		{
			var f = b.webkitGetAsEntry();
			f && (f.isFile ? k.push(b.getAsFile()) : d.push(c(f).done(function ()
			{
				d.pop(), 0 === d.length && e.success()
			})))
		}) : k = a.files, 0 === d.length && e.success()), e
	}
	...
	function e(a)
	{
		...
		{
			qq(a).hasAttribute(i) && qq(a).hide(), qq(a).removeClass(h.classes.dropActive), d(e.dataTransfer, c).done(function ()
			{
				b(k, c)
			})
		}});
		...
		{
			c.dispose()
		}), qq(a).hasAttribute(i) && qq(a).hide(), j.push(c), c
	}
	...
	function g()
	{
		...
		{
			var d = e(c);
			!a.length || qq.ie() && !qq.ie10() || l.attach(document, "dragenter", function (b)
			{
		...
		}), l.attach(document, "dragleave", function (b)
		{
			qq.FineUploader.prototype._leaving_document_out(b) && qq.each(a, function (a, b)
			{
				qq(b).hasAttribute(i) && qq(b).hide()
			})
		}), l.attach(document, "drop", function (b)
		{
	...
	var h, i = "qq-hide-dropzone", j = [], k = [], l = new qq.DisposeSupport;
	return h = {dropZoneElements: [], allowMultipleItems: !0, classes: {dropActive: null}, callbacks: new qq.DragAndDrop.callbacks}, qq.extend(h, a, !0), g(), {setupExtraDropzone: function (a)
	{
	...
	{
		l.dispose(), qq.each(j, function (a, b)
		{
			b.dispose()
		})
	}}
...
}(), qq.ImageGenerator = function (a)
{
	...
	function h(b, c)
	{
		...
		{
			b.onload = null, b.onerror = null, a("Problem drawing preview!", "error"), c.failure(b, "Problem drawing preview!")
		}
	}
	...
	{
		var f = b || c;
		return b(d) ? h(d, e) : c(d) ? i(d, e) : (e.fail(d), a(qq.format("Element container of type {} is not supported!", d.tagName), "error")), f
	}
	...
	var o;
	return o = {generate: function (b, c, d)
	{
	...
	}}, o._testing = {}, o._testing.isImg = b, o._testing.isCanvas = c, o._testing.isCrossOrigin = g, o._testing.determineMimeOfFileName = f, o
}, qq.Exif = function (a, b)
{
	...
	function j(a, b)
	{
		var d = 16, e = qq.extend([], l), f = {};
		return qq.each(b, function (b, g)
		{
			var h, i, j, k = g.slice(0, 4), l = a ? c(k) : parseInt(k, 16), n = e.indexOf(l);
			return n >= 0 && (i = m[l].name, j = m[l].bytes, h = g.slice(d, d + 2 * j), f[i] = a ? c(h) : parseInt(h, 16), e.splice(n, 1)), 0 === e.length ? !1 : void 0
		}), f
	}

	var k, l = [274], m = {274: {name: "Orientation", bytes: 2}};
	...
	}}, k._testing = {}, k._testing.parseLittleEndian = c, k
...
}, qq.UiEventHandler = function (a, b)
{
	...
	return qq.extend(b, {getFileIdFromItem: function (a)
	{
		return a.qqFileId
...
}, qq.DeleteRetryOrCancelClickHandler = function (a)
{
	...
	function b(a, b)
	{
		if (e.templating.isCancel(a) || e.templating.isRetry(a) || e.templating.isDelete(a))
		{
			var d = e.templating.getFileId(a);
			qq.preventDefault(b), e.log(qq.format("Detected valid cancel, retry, or delete click event on file '{}', ID: {}.", e.onGetName(d), d)), c(a, d)
		}
	}

	function c(a, b)
	{
		e.templating.isDelete(a) ? e.onDeleteFile(b) : e.templating.isCancel(a) ? e.onCancel(b) : e.onRetry(b)
	}

	var d = {}, e = {templating: null, log: function ()
	{
	...
	qq.extend(e, a), e.eventType = "click", e.onHandled = b, e.attachTo = e.templating.getFileList(), qq.extend(this, new qq.UiEventHandler(e, d))
...
}, qq.FilenameEditHandler = function (a, b)
{
	...
	var h, i = {templating: null, log: function ()
	{
	...
	return qq.extend(i, a), i.attachTo = i.templating.getFileList(), h = qq.extend(this, new qq.UiEventHandler(i, b)), qq.extend(b, {handleFilenameEdit: function (a, b, d)
	{
		var e = i.templating.getEditInput(a);
		i.onEditingStatusChange(a, !0), e.value = c(a), d && e.focus(), f(e, a), g(e, a)
	}}), h
}, function (a)
{
	"use strict";
	...
	function g(b, c)
	{
		...
		a.each(c._options.callbacks, function (b, c)
		{
			...
			e = /^on(\w+)/.exec(b)[1], e = e.substring(0, 1).toLowerCase() + e.substring(1), f = l, d[b] = function ()
			{
				...
				{
					h.push(k(b))
				}), b = c.apply(this, g), d = f.triggerHandler(e, h), null != b ? b : d
			}
...
}(jQuery), qq.s3 = qq.s3 || {}, qq.s3.util = qq.s3.util || function ()
{
	...
	}, getPolicy: function (a)
	{
		...
		return b.expiration = qq.s3.util.getPolicyExpirationDate(h), c.push({acl: f}), c.push({bucket: d}), g && c.push({"Content-Type": g}), i && c.push({success_action_status: i.toString()}), k && c.push({success_action_redirect: k}), n && c.push({"x-amz-storage-class": "REDUCED_REDUNDANCY"}), c.push({key: e}), qq.each(j, function (a, b)
		{
			var d = qq.s3.util.AWS_PARAM_PREFIX + a, e = {};
			if (a != 'qqfilename')
			{
				e[d] = encodeURIComponent(b);
				c.push(e);
			}
	...
	}, generateAwsParams: function (a, b)
	{
		...
		return /*c.key = a.key, */c.AWSAccessKeyId = i, /*g && (c["Content-Type"] = g),*/ k && (c.success_action_status = 201), l && (c.success_action_redirect = l), m && (c["x-amz-storage-class"] = "REDUCED_REDUNDANCY"), c.acl = j, qq.each(d, function (a, b)
		{
			if (a != 'qqfilename')
			{
		...
		{
			c.policy = a.policy, c.key = a.key, c.signature = a.signature, e.success(c)
		}, function (a)
		{
...
{
	var b = {request: {accessKey: null, uuidName: "uuid"}, objectProperties: {acl: "private", key: "uuid", reducedRedundancy: !1}, signature: {endpoint: null, customHeaders: {}}, uploadSuccess: {endpoint: null, params: {}, customHeaders: {}}, iframeSupport: {localBlankPagePath: null}, chunking: {partSize: 5242880}, resume: {recordsExpireIn: 7}, cors: {allowXdr: !0}};
...
}}), qq.s3.SignatureAjaxRequestor = function (a)
{
	...
	}}, paramsStore: e.paramsStore, maxConnections: e.maxConnections, customHeaders: e.signatureSpec.customHeaders, log: e.log, onComplete: b, cors: e.cors, successfulResponseCodes: {POST: [200, 201]}}), {getSignature: function (a, b)
	{
...
}, qq.s3.UploadSuccessAjaxRequester = function (a)
{
	...
	{
		return e.endpoint
	}}, paramsStore: e.paramsStore, maxConnections: e.maxConnections, customHeaders: e.customHeaders, log: e.log, onComplete: b, cors: e.cors, successfulResponseCodes: {POST: [200,201]}}), {sendSuccessRequest: function (a, b)
	{
...
}, qq.s3.InitiateMultipartAjaxRequester = function (a)
{
	...
	function b(a)
	{
		...
		return e["x-amz-date"] = (new Date).toUTCString(), e["Content-Type"] = h.getContentType(a), e["x-amz-acl"] = h.acl, h.reducedRedundancy && (e["x-amz-storage-class"] = "REDUCED_REDUNDANCY"), e[qq.s3.util.AWS_PARAM_PREFIX + h.filenameParam] = encodeURIComponent(h.getName(a)), qq.each(h.paramsStore.getParams(a), function (a, b)
		{
	...
	var e, f, g = {}, h = {filenameParam: "qqfilename", method: "POST", endpointStore: null, paramsStore: null, signatureSpec: null, accessKey: null, acl: "private", reducedRedundancy: !1, maxConnections: 3, getContentType: function ()
	{
	}, getKey: function ()
	{
	}, getName: function ()
	...
	return qq.extend(h, a), f = new qq.s3.SignatureAjaxRequestor({signatureSpec: h.signatureSpec, cors: h.cors, log: h.log}), e = new qq.AjaxRequestor({method: h.method, contentType: null, endpointStore: h.endpointStore, maxConnections: h.maxConnections, log: h.log, onComplete: d, successfulResponseCodes: {POST: [201]}}), {send: function (a)
	{
...
}, qq.s3.UploadHandlerForm = function (a, b, c, d)
{
	...
	{
		var b = q.getParams(a);
		return b[p] = j.getName(a), qq.s3.util.generateAwsParams({endpoint: r.getEndpoint(a), params: b, key: k[a].key, accessKey: s, acl: t, minFileSize: v.minSizeLimit, maxFileSize: v.maxSizeLimit, successRedirectUrl: x, reducedRedundancy: u, log: l}, qq.bind(y.getSignature, this, a))
	}
	...
	function g(b, c)
	{
		...
		{
			var b = z.initFormForUpload({method: e, endpoint: g, params: a, paramsInBody: !0, targetName: c.name});
			d.success(b)
		}, function (a)
		{
	...
	var j, k = [], b = b, l = d, m = a.onComplete, n = a.onUpload, o = a.getKeyName, p = a.filenameParam, q = a.paramsStore, r = a.endpointStore, s = a.accessKey, t = a.objectProperties.acl, u = a.objectProperties.reducedRedundancy, v = a.validation, w = a.signature, x = a.iframeSupport.localBlankPagePath, y = new qq.s3.SignatureAjaxRequestor({signatureSpec: w, cors: a.cors, log: l}), z = {};
	if (void 0 === x)throw new Error("successRedirectEndpoint MUST be defined if you intend to use browsers that do not support the File API!");
	return j = new qq.UploadHandlerFormApi(z, k, !1, "file", a.onCancel, c, l), qq.extend(j, {upload: function (a)
	{
...
}, qq.s3.UploadHandlerXhr = function (a, b, c, d)
{
	...
	{
		var b = J.getFile(a);
		K[a].type = b.type, $.createXhr(a), y(a) ? (void 0 === K[a].loaded && (K[a].loaded = 0), z(a)) : (K[a].loaded = 0, o(a))
	}
	...
	{
		var b = R.getParams(a);
		return b[Q] = J.getName(a), qq.s3.util.generateAwsParams({endpoint: S.getEndpoint(a), params: b, type: K[a].type, key: f(a), accessKey: T, acl: U, expectedStatus: L, minFileSize: W.minSizeLimit, maxFileSize: W.maxSizeLimit, reducedRedundancy: V, log: d}, qq.bind(_.getSignature, this, a))
	}
	...
	{
		var b, c;
		Z && void 0 === f(a) && (b = x(a), c = localStorage.getItem(b), c && (d(qq.format("Identified file with ID {} and name of {} as resumable.", a, J.getName(a))), c = JSON.parse(c), K[a].uuid = c.uuid, g(a, c.key), K[a].loaded = c.loaded, K[a].chunking = c.chunking))
	}

	function s(a)
	{
		var b, c;
		Z && (b = x(a), c = {name: J.getName(a), size: J.getSize(a), uuid: J.getUuid(a), key: f(a), loaded: K[a].loaded, chunking: K[a].chunking, lastUpdated: Date.now()}, localStorage.setItem(b, JSON.stringify(c)))
	}

	function t(a)
	{
		var b;
		return Z && (b = x(a), b && localStorage.getItem(b)) ? (localStorage.removeItem(b), !0) : !1
	}

	function u(a)
	{
		Z && qq.each(localStorage, function (b, c)
		{
	...
	{
		var b;
		return K[a].chunking || (K[a].chunking = {}, b = $.getTotalChunks(a), b > 1 ? (K[a].chunking.enabled = !0, K[a].chunking.parts = b) : K[a].chunking.enabled = !1), K[a].chunking.enabled
	}
	...
	function F(b)
	{
		var c = A(b), e = J.getName(b), f = K[b].xhr, g = C(b), h = J.getSize(b), j = $.getChunkData(b, c);
		G(b).then(function (c)
		{
			a.onUploadChunk(b, e, $.getChunkDataForCallback(j)), f.upload.onprogress = function (c)
			{
				if (c.lengthComputable)
				{
	...
	function G(b)
	{
		...
		return d["x-amz-date"] = i, c = {headers: "PUT\n\n\n\nx-amz-date:" + i + "\n" + "/" + g + "/" + h + j}, ab.getSignature(b, c).then(function (b)
		{
	...
	function H(a)
	{
		return K[a].chunking.uploadId ? (new qq.Promise).success(K[a].chunking.uploadId) : bb.send(a).then(function (b)
		{
	...
	function I(a)
	{
		var b, c = A(a), e = K[a].xhr, f = l(a), g = $.getChunkData(a, c);
	...
	{
		return J.getName(a)
	}}), cb = new qq.s3.CompleteMultipartAjaxRequester({endpointStore: S, signatureSpec: X, accessKey: a.accessKey, cors: a.cors, log: d, getKey: function (a)
	{
		return e(a)
	}}), db = new qq.s3.AbortMultipartAjaxRequester({endpointStore: S, signatureSpec: X, accessKey: a.accessKey, cors: a.cors, log: d, getKey: function (a)
	{
		return e(a)
	}});
	return J = new qq.UploadHandlerXhrApi($, K, Y ? a.chunking : null, n, a.onCancel, c, d), w(), qq.override(J, function (a)
	{
		...
		}, expunge: function (b)
		{
			...
			void 0 !== c && d && db.send(b, c), a.expunge(b)
		}, getThirdPartyFileId: function (a)
		{
			return f(a)
		}}
...
}, qq.extend(qq.s3.FineUploader.prototype, qq.s3.FineUploaderBasic.prototype), qq.extend(qq.s3.FineUploader.prototype, qq.uiPublicApi), qq.extend(qq.s3.FineUploader.prototype, qq.uiPrivateApi), qq.extend(qq.s3.FineUploader.prototype, {_onComplete: function (a)
{
	...
	return qq.isPromise(b) && (this._templating.hideProgress(a), this._templating.setStatusText(a, this._options.text.waitingForResponse)), b
}}), function (a)
{
	"use strict";
